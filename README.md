# Token Monitor

App de barra de menú para macOS que muestra los límites de uso de Codex, Claude Code y Antigravity.

## Uso

1. Inicia sesión en Codex, Claude Code y Antigravity CLI en este Mac.
2. Ejecuta `./scripts/install-app.sh`.
3. Abre **Token Monitor** desde Aplicaciones y pulsa su icono en la barra de menú.

La app se actualiza cada cinco minutos; Claude se consulta como máximo cada quince minutos o cuando termine la espera que indique su servicio. La cifra de la barra muestra el promedio de uso semanal de Codex y Claude; si falta una lectura reciente, queda vacía. Antigravity aparece en el panel y en el reparto, pero no altera ese promedio. El panel muestra los porcentajes y reinicios de cada ventana, no un saldo fijo de tokens. Si Claude limita las consultas, su última lectura permanece visible con su antigüedad y la hora del próximo intento.

En **Nueva tarea**, escribe lo que necesitas y elige la carpeta del proyecto. Token Monitor consulta las cuotas y abre una sesión de la IA con más capacidad disponible. Compara el porcentaje libre de la ventana más ajustada de cada cuenta; no existen saldos absolutos de tokens comparables entre suscripciones.

Para repartir una tarea entre actividades desde otro agente o la terminal, ejecuta `'/Applications/Token Monitor.app/Contents/MacOS/token-route' choose` antes de cada actividad. `token-route run 'actividad'` la asigna y ejecuta con una sola IA, escogida de nuevo en ese momento. Antigravity ejecuta las tareas asignadas con Gemini 3.1 Pro Low, cuya cuota «Gemini Models» lee la app. Las tareas escritas directamente en las apps de Codex o Claude ya han empezado en esa IA; las instrucciones globales las derivan cuando el monitor elige otra.

## Desarrollo

`./scripts/test.sh` comprueba el análisis de las respuestas. `./scripts/build-app.sh` crea la app en `dist/`.

Basada en [UsageBar de Lucas Barake](https://github.com/lucas-barake/usagebar), bajo licencia MIT; consulta [LICENSE](LICENSE). Esta copia no descarga ni instala actualizaciones del proyecto original.
